...
class NetworkObject extends GameEntity {
  ...
  void display() {
    ...
    if (showScanResult) {
      ...
      noStroke(); ellipse(x+w/2, y-h/2, 16, 16);
  ...
  void drawPacket() {
    color bg = scanned
      ...
      : color(20,40,80);
    fill(bg); stroke(0,180,255); strokeWeight(1);
    rect(x-w/2, y-h/2, w, h, 4);
    fill(180,220,255); textSize(11); textAlign(CENTER,CENTER); noStroke();
    text(id, x, y-5);
    fill(0,140,180,130); textSize(7);
...
void gameReleased() {
  ...
  if (isInZone(selectedObj.x, selectedObj.y, scannerX, scannerY, scannerW, scannerH)) {
    if (!isScanning) {
      ...
      stackPush(selectedObj); // push to stack
    }
  ...
  // INCINERATOR
    ...
    burnSelectedObject(); // Nicole's function
    isScanning  = false;
    selectedObj = null;
...
void drawScannerAndIncinerator() {
  ...
  // Scanner box
  stroke(0,200,255); strokeWeight(2); noFill();
  rect(scannerX, scannerY, scannerW, scannerH, 8);
  fill(200,230,255); noStroke(); textSize(11); textAlign(CENTER,TOP);
  ...
  if (isScanning) {
    ...
    fill(40); rect(scannerX+10, scannerY+scannerH-20, scannerW-20, 12, 3);
...
void drawStatsPanel() {
  ...
  fill(15,25,45,215); stroke(0,140,220); strokeWeight(1);
  rect(px, py, pw, ph, 6);

  image(imgHealth, px+8, py+6, 18, 18);
  fill(0,200,255); noStroke(); textSize(12); textAlign(LEFT,TOP);
  ...
  fill(180); noStroke(); textSize(10); textAlign(LEFT,CENTER);
...
void drawStatBar(String label, float val, float maxVal,
  ...
  fill(40); rect(x, y+14, bw, bh, 3);
  fill(c); rect(x, y+14, map(constrain(val,0,maxVal),0,maxVal,0,bw), bh, 3);
}

// Live animated threat gauge
void drawThreatDial(float cx, float cy, float r) {
  fill(30); stroke(80); strokeWeight(1);
